...
function InputGoals() {
  ...
  // States to manage the selected file, prediction results, and errors
  ...
  const handleIllustrationSubmit = async () => {
    ...
    const messageToSend: string = messageToSent || "";

    try {
      setLoading(true); // Show loading spinner
      const responseMessage = await sendMessageToOpenAI(messageToSend);
      setLoading(false); // Hide loading spinner
      navigate("/resultsPage", { state: { response: responseMessage } }); // Navigate to ResultsPage with response
    } catch (error) {
